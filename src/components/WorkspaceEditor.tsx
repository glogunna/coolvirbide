...
export const WorkspaceEditor: React.FC<WorkspaceEditorProps> = ({ project }) => {
  ...
  // CRITICAL: Protected items that cannot be deleted
  const protectedItems = new Set([
    'workspace', 'workspace-view', 'sharedStorage', 'privateStorage', 'soundService', 
    'mediaService', 'uiService', 'threedStorage', 'dataVault', 'replicatedStorage',
  ...
  // CRITICAL: Listen for explorer object selection
  useEffect(() => {
    const handleExplorerSelection = (event: CustomEvent) => {
      ...
      if (foundObject) {
        ...
        if (foundObject.mesh && sceneRef.current) {
          highlightObject(foundObject.mesh);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    // CRITICAL: Load workspace objects from the actual project
    const loadWorkspaceObjects = () => {
      console.log('[WORKSPACE] Loading workspace objects from project:', project.services.workspace.objects);
      ...
      // Load actual workspace objects from project
      if (project.services.workspace.objects) {
  ...
  const addObject = (type: string, parentId?: string) => {
    ...
    // CRITICAL: Update the project data AND notify file explorer
    if (parentId) {
    ...
    // Add to workspace objects in project
    if (!project.services.workspace.objects) {
      project.services.workspace.objects = [];
    }
    project.services.workspace.objects.push(newObject);

    setObjects(prev => [...prev, newObject]);
    setShowAddMenu(false);

    // CRITICAL: Notify file explorer to refresh
...
console.log("Configuration loaded");`;
  ...
  const deleteObject = (id: string) => {
    // CRITICAL: Check if object is protected
    if (protectedItems.has(id)) {
      console.log('[WORKSPACE] Cannot delete protected item:', id);
      return;
    }
    ...
    if (objectToDelete) {
      ...
      // CRITICAL: Remove from project data
      if (project.services.workspace.objects) {
        project.services.workspace.objects = project.services.workspace.objects.filter((obj: any) => obj.id !== id);
      }
  ...
  const renderObjectHierarchy = (obj: any, depth = 0) => {
    ...
    const isProtected = protectedItems.has(obj.id);
      ...
      <div key={obj.id}>
        ...
        >
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            {editingName === obj.id && !isProtected ? (
              <input
                type="text"
                value={obj.name}
      ...
      <div className="flex-1 bg-gray-900 relative">
        ...
        <div className="absolute bottom-4 left-4 bg-black bg-opacity-70 text-white p-3 rounded-lg text-sm">
          <div className="space-y-1">
            ...
            <div className="text-purple-400">✓ Explorer selection highlights objects!</div>
          </div>
        </div>
      </div>
